...
-- =========================================================
-- [CRIADO] Tabelas: ponto_descarte_adequado e
--                   ponto_descarte_residuo
-- Infraestrutura de orientação cidadã e análise oferta/demanda.
-- =========================================================

CREATE TABLE ponto_descarte_adequado (
    id_ponto_descarte       INT          PRIMARY KEY AUTO_INCREMENT,
    id_logradouro           INT          NOT NULL,
    nome_local              VARCHAR(120) NOT NULL,
    tipo_ponto              VARCHAR(60)  NOT NULL,
    /*
      Valores esperados: 'ECOPONTO', 'PEV', 'COOPERATIVA',
      'FARMACIA', 'ESCOLA', 'SUPERMERCADO', 'OUTRO'
    */
    ...
    CONSTRAINT fk_ponto_descarte_logradouro
        FOREIGN KEY (id_logradouro)
        REFERENCES logradouro(id_logradouro)
);

CREATE TABLE ponto_descarte_residuo (
    id_ponto_descarte   INT NOT NULL,
    id_tipo_residuo     INT NOT NULL,

    PRIMARY KEY (id_ponto_descarte, id_tipo_residuo),

    CONSTRAINT fk_pdr_ponto
        FOREIGN KEY (id_ponto_descarte)
        REFERENCES ponto_descarte_adequado(id_ponto_descarte),

    CONSTRAINT fk_pdr_residuo
        FOREIGN KEY (id_tipo_residuo)
        REFERENCES tipo_residuo(id_tipo_residuo)
);

/*
REGRAS DE NEGÓCIO — PONTOS DE DESCARTE:
  1. Ao registrar uma ocorrência, o sistema sugere ao denunciante
     os pontos ativos mais próximos (consulta geoespacial por
     latitude/longitude) que aceitam o tipo de resíduo informado.
     Isso integra função repressiva (denúncia) com função educativa
     (orientação), alinhado à ODS 11 e ODS 12.

  2. A coluna cor_padrao_conama de tipo_residuo alimenta a
     sinalização visual do mapa de pontos — exibe a cor da
     lixeira correta para cada tipo de resíduo.

  3. Pontos com ativo = FALSE não aparecem nas consultas públicas
     mas são preservados para análise histórica de cobertura.

ANÁLISE OFERTA VS DEMANDA (sem nova tabela de equipes):
  O objetivo do sistema é identificar pontos críticos por
  localização — não monitorar equipes operacionais.
  A análise de oferta e demanda é realizada nos dashboards como:

  DEMANDA → volume e frequência de ocorrências por bairro/região
             (já capturado em fato_ocorrencia + dim_localizacao)

  OFERTA  → quantidade e distribuição de pontos de descarte
             adequado ativos por bairro/região
             (nova estrutura ponto_descarte_adequado)

  Interpretação analítica:
    Muitas ocorrências + poucos pontos de descarte
    → déficit de infraestrutura → ação: expansão de cobertura.

    Muitos pontos de descarte + alta reincidência
    → problema comportamental → ação: campanha de sensibilização.
*/